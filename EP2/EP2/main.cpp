...
void Emulador_de_MIPS(int opcao, MemoriaRAM *ram,UnidadeDeControle *Unit);
void TelaMemoria(MemoriaRAM *ram);
...
int main() {
    ...
    Emulador_de_MIPS(opcao, ram, Unit);
    ...
    while (opcao != 0) {
        ...
        if(opcao == 3) { //Executar proxima instrucao
            try {
                ...
                cout << "PC: " << Unit->getPC() << endl << endl << endl;
